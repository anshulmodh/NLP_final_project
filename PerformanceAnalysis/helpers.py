...
def prepare_dataset_nli(examples, tokenizer, hypoth_only, premise_only, max_seq_length=None):
    max_seq_length = tokenizer.model_max_length if max_seq_length is None else max_seq_length

    premise, hypothesis, label = "premise", "hypothesis", "label"
    # premise, hypothesis, label = "sentence1", "sentence2", "gold_label"

    label_mapping = {"entailment": 0, "neutral": 1, "contradiction": 2, "-": -1}

    if hypoth_only:
        examples[premise] = [""] * len(examples[premise])

    if premise_only:
        examples[hypothesis] = [""] * len(examples[hypothesis])
    try:
        ...
        print(e)
        f = open("log.txt", 'w')
        f.write(json.dumps(examples.data))
        1/0

    # tokenized_examples['label'] = [label_mapping[x] for x in examples[label]]
...
def compute_accuracy(eval_preds: EvalPrediction):
    ...
    confusion, report = compute_stats(predictions, labels)
    return {
...
def compute_stats(preds, labels):
